...
//var puntiTot = 10;
var onOff = true;   // dice se un bottone si puÃ² accendere o no
var ind = true;   // permette di spegnere i bottoni
var diag = false;   // dice se si Ã¨ andati in diagonale
var back = new Array();   // array contenente per ogni bottone se puÃ² essere spento (per tornare indietro)
var a=0;
var r=0;
...
function tornaIndietro (el_id){
	...
	if (document.getElementById('287b').style.opacity=="0.5" || document.getElementById('287b').style.opacity=="0.6")
		return;
	if (diag == true)
		return;
	if (a > 0)
...
// Per far apparire che il mouse e' passato sopra un bottone.
function changeColor (element_id){
	if (document.getElementById('99b').style.opacity!="0.5" && document.getElementById('99b').style.opacity!="0.6")
		return;
	if (u > 0)
		return;
	if (ind == false)
...
function controlloEst (element_id){
	...
	if (document.getElementById('99b').style.opacity=="0.5" || document.getElementById('99b').style.opacity=="0.6" && k==383)
		window.location.reload();
}


// Rende piÃ¹ trasparenti gli ultimi due bottoni su cui Ã¨ passato il mouse.
